...
def load_roll_args_file(file_path: str) -> RollArgs:
    """从 JSON 文件加载 roll 参数。"""
    with open(file_path, 'r', encoding='utf-8') as f:
        json_str = f.read()
    return load_roll_args(json_str)


def load_roll_args(json_str_obj: Union[str, any]) -> RollArgs:
...
def read_oi_csv(csv_path: str, dt_from: datetime, dt_to: datetime) -> pd.DataFrame:
...
def aggr_trade_position(position_df_list: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    aggr_df = aggr_df[aggr_df['position_diff'] != 0.0]
    aggr_df = aggr_df[['dt', 'dt_raw', 'trade_args_open_count', 'position', 'weight_sum']]
    return aggr_df


def aggr_set_meta(aggr_df: pd.DataFrame, roll_args: RollArgs) -> pd.DataFrame:
    aggr_df['roll_args_id'] = roll_args.roll_args_id
    aggr_df['top'] = roll_args.roll_top
    return aggr_df
...
roll_args = load_roll_args_file(f'{DATA_DIR}/roll/roll_test.json')
trigger_df = merge_trade_trigger(
        roll_args.trade_weight,
        roll_args.trade_trigger)
read_oi_df = read_oi_csv('../data/fact/oi_159915_20250101_20250709.csv',
                        roll_args.input_dt_from, roll_args.input_dt_to)
converted_df = convert_oi_df(read_oi_df)
...
for trade_args_id, trade_df in split_trade_args_dict.items():
    ...
    for cycle_df in trade_cycles:
        pass
        # print(f"Cycle DataFrame for trade_args_id {trade_args_id}:\n{cycle_df.head(10)}\n{cycle_df.tail(10)}\n{'-'*40}")

aggr_df = aggr_trade_position(position_dfs)
aggr_df = aggr_set_meta(aggr_df, roll_args)
aggr_df = aggr_cut(aggr_df)
print(f"Aggregated trade positions DataFrame:\n{aggr_df.head(20)}")
