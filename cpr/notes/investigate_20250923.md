...
根据 cpr.roll_merged + etf roll profit，这天的 spot 收益是 -1.48% .  
...
根据 sig_worth.py 提出的回测，今天的理论亏损应该是 2.4% ，这样来说的话，那么亏损的计算是正确的。  
那么就是 roll_merged 表格提出的 SQL 回测不对了，roll_merged 是怎么计算收益的？  
我发现问题出在之前的 SQL 盈利核算上，我不能用 avg(profit) 而是应该使用 sum(profit) / top_count ，因为一个参数可以交易很多次。  
所以经过修正之后的代码可以得到 -2.5% 的正确结果。
...
根据 Time Value 的总和计算，我确定不是滑点的原因，那么只有可能是因为 ETF 的回测交易价格和下单的时候 ETF 价格完全对不上了。  
有 10k 的亏损都是 intrinsic value 产生的，这个数值只和 ETF 价格有关。  
