...
第二个脚本输出 `backtest/data/output/pnl_xxxx.csv` 文件，之后可以用这个文件绘图。  
...
一个更加好的做法是定期刷新它的最大持仓量，我们需要在 Underlying 价格移动 1% 以上，或者每隔十分钟计算新的最大持仓量数字，这样可能风险更加可控一些。用 Max Exposure 固定最大持仓量。
...
### 做空对冲股票多头
1. syn_i12_short  持续空头，固定敞口，最小下单量 5 手，大致在价格变动 1.5% 的时候触发。
2. syn_i14_short  持续空头，固定敞口，最小下单量 3 手，大致在价格变动 1% 的时候触发。
3. syn_i15_short  持续空头，固定持仓量，敞口随价格变化。
